...
Setting `hooks: true` will generate `useQuery` and `useMuation` hook exports. If you also want `useLazyQuery` hooks generated or generally more control, you can also pass an object in the form ` { queries: boolean; lazyQueries: boolean; mutations: boolean }` in.
